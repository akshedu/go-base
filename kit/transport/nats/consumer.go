...
func NewConsumer(connstr string, st *Stream, options ...ConsumerOption) (*Consumer, error) {
	var (
		err  error
		cc   *natn.Conn
	...
	// Create a consumer
	info, err := st.js.AddConsumer(st.cfg.Name, c.cfg)
...
func (c *Consumer) GetConsumerInfo() (*natn.ConsumerInfo, error) {
	return c.info, nil
}

func (c *Consumer) UpdateConsumer(st *Stream, options ...ConsumerOption) (*Consumer, error) {
	for _, fn := range options {
		fn(c)
	}

	// Update the consumer
	info, err := st.js.UpdateConsumer(st.cfg.Name, c.cfg)
...
func (c *Consumer) DeleteConsumer(st *Stream) error {

	// Delete the consumer
	err := st.js.DeleteConsumer(st.cfg.Name, c.cfg.Durable)
